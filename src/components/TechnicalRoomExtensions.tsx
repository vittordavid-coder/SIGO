                      ...
                      <div className="flex gap-2 self-end sm:self-auto items-center">
                        ...
                        {!readonly && (
                          <Button 
                            ...
                            className="font-extrabold uppercase text-[10px] tracking-widest flex items-center gap-1.5"
                          >
                            <Trash2 className="w-3.5 h-3.5" /> EXCLUIR RDO
                          </Button>
                        )}
                      </div>
                    </div>
...
export function PluviometryView({ contract, records, onAdd, onUpdate, readonly }: PluviometryViewProps) {
  ...
  // Modal State for Double-Click editing
  const [selectedDayForModal, setSelectedDayForModal] = useState<number | null>(null);
  const [modalNightStatus, setModalNightStatus] = useState<'Bom' | 'Chuvoso' | 'Impraticável'>('Bom');
  const [modalMorningStatus, setModalMorningStatus] = useState<'Bom' | 'Chuvoso' | 'Impraticável'>('Bom');
  const [modalAfternoonStatus, setModalAfternoonStatus] = useState<'Bom' | 'Chuvoso' | 'Impraticável'>('Bom');
  const [modalRainfallMm, setModalRainfallMm] = useState<number>(0);

  const handleOpenModal = (day: number) => {
    setSelectedDayForModal(day);
    const rec = getRecordForDay(day);
    setModalNightStatus(rec?.nightStatus || 'Bom');
    setModalMorningStatus(rec?.morningStatus || 'Bom');
    setModalAfternoonStatus(rec?.afternoonStatus || 'Bom');
    setModalRainfallMm(rec?.rainfallMm || 0);
  };

  const handleSaveModal = () => {
    if (selectedDayForModal === null || readonly) return;
    const day = selectedDayForModal;
    ...
    if (existing) {
      onUpdate({
        ...existing,
        nightStatus: modalNightStatus,
        morningStatus: modalMorningStatus,
        afternoonStatus: modalAfternoonStatus,
        rainfallMm: modalRainfallMm,
      });
    } else {
      onAdd({
        contractId: contract.id,
  ...
  const getPluviometryHTMLReport = () => {
    ...
    let leadingCellsHtml = '';
    for (let i = 0; i < firstDayOfWeek; i++) {
      leadingCellsHtml += `<div style="background: #f8fafc; border: 1px dashed #e2e8f0; border-radius: 6px; min-height: 64px; opacity: 0.35;"></div>`;
    }

    const todayDate = new Date();

    const dayCellsHtml = monthDays.map(day => {
      const rec = getRecordForDay(day);
      const night = rec?.nightStatus || 'Bom';
      const morning = rec?.morningStatus || 'Bom';
      const afternoon = rec?.afternoonStatus || 'Bom';
      const rainfall = rec?.rainfallMm || 0;

      const isImpraticable = morning === 'Impraticável' || afternoon === 'Impraticável';
      const isRainy = morning === 'Chuvoso' || afternoon === 'Chuvoso' || night === 'Chuvoso' || rainfall > 0;
      const dayOfWeek = new Date(currentYear, currentMonth, day).getDay();
      const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
      const isToday = day === todayDate.getDate() && currentMonth === todayDate.getMonth() && currentYear === todayDate.getFullYear();

      let cellBg = 'white';
      let borderColor = isToday ? '#2563eb' : '#cbd5e1';
      let badgeHtml = '<span style="background: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; padding: 2px 5px; border-radius: 4px; font-size: 8px; font-weight: bold; text-transform: uppercase;">☀️ Produtivo</span>';

      if (isImpraticable) {
        cellBg = '#fef2f2';
        borderColor = isToday ? '#2563eb' : '#fca5a5';
        badgeHtml = '<span style="background: #dc2626; color: white; padding: 2px 5px; border-radius: 4px; font-size: 8px; font-weight: bold; text-transform: uppercase;">🛑 Impraticável</span>';
      } else if (isRainy) {
        cellBg = '#eff6ff';
        borderColor = isToday ? '#2563eb' : '#93c5fd';
        badgeHtml = '<span style="background: #dbeafe; color: #1e40af; border: 1px solid #93c5fd; padding: 2px 5px; border-radius: 4px; font-size: 8px; font-weight: bold; text-transform: uppercase;">🌧️ Sob Chuva</span>';
      } else if (isWeekend) {
        cellBg = '#f8fafc';
        borderColor = isToday ? '#2563eb' : '#e2e8f0';
        badgeHtml = '<span style="color: #94a3b8; font-size: 8px; font-weight: bold; text-transform: uppercase;">Fim de Semana</span>';
      }

      const getEmoji = (st: string) => st === 'Bom' ? '☀️' : st === 'Chuvoso' ? '🌧️' : '🛑';

      return `
        <div style="background-color: ${cellBg}; border: ${isToday ? '2px' : '1px'} solid ${borderColor}; border-radius: 6px; padding: 5px; min-height: 64px; display: flex; flex-direction: column; justify-content: space-between; box-sizing: border-box; ${isToday ? 'background: #f0f7ff;' : ''}">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
            <div style="display: flex; align-items: center; gap: 4px;">
              <span style="font-weight: 900; font-family: monospace; font-size: 11px; color: ${isToday ? '#2563eb' : '#0f172a'};">${String(day).padStart(2, '0')}</span>
            </div>
            ${badgeHtml}
          </div>
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 2px; text-align: center; background: rgba(255,255,255,0.9); padding: 3px; border-radius: 4px; border: 1px solid rgba(0,0,0,0.06); margin: 2px 0;">
            <div><div style="font-size: 6px; font-weight: 800; color: #64748b; text-transform: uppercase;">NOITE</div><div style="font-size: 10px;">${getEmoji(night)}</div></div>
            <div style="border-left: 1px solid #e2e8f0; border-right: 1px solid #e2e8f0;"><div style="font-size: 6px; font-weight: 800; color: #64748b; text-transform: uppercase;">MANHÃ</div><div style="font-size: 10px;">${getEmoji(morning)}</div></div>
            <div><div style="font-size: 6px; font-weight: 800; color: #64748b; text-transform: uppercase;">TARDE</div><div style="font-size: 10px;">${getEmoji(afternoon)}</div></div>
          </div>
          <div style="display: flex; justify-content: space-between; align-items: center; font-size: 8px; font-weight: 800;">
            ${rainfall > 0 ? `<span style="background: #dbeafe; color: #1e40af; padding: 1px 4px; border-radius: 3px; font-family: monospace; font-weight: bold;">🌧️ ${rainfall.toFixed(1)} mm</span>` : `<span style="color: #94a3b8; font-size: 8px;">0.0 mm</span>`}
          </div>
        </div>
      `;
    }).join('');
    ...
    return `
      ...
      <html>
        <head>
          <title>Relatorio_Pluviometrico_${contract.contractNumber || 'SYNERA'}</title>
          <style>
        ...
        <body>
          <div class="header">
            <div>
              <h1 style="font-size: 16px; font-weight: 900; color: #1e3a8a; margin: 0; text-transform: uppercase;">SYNERA - Gestão e Planejamento</h1>
          ...
          <div class="kpi-container">
            ...
            <div class="kpi-box">
              <div class="kpi-title">Dias Produtivos</div>
              <div class="kpi-val">${bDays} dias</div>
            </div>
          </div>

          <div style="background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 6px 10px; margin-bottom: 10px; font-size: 8.5px; display: flex; align-items: center; justify-content: space-between;">
            <span style="font-weight: 800; color: #1e3a8a; text-transform: uppercase;">Legenda Climática:</span>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
              <span style="display: inline-flex; align-items: center; gap: 3px;"><span style="background: #dc2626; color: white; padding: 1px 4px; border-radius: 3px; font-weight: 800; font-size: 8px;">🛑 IMPRATICÁVEL</span> Paralisação da Obra</span>
              <span style="display: inline-flex; align-items: center; gap: 3px;"><span style="background: #dbeafe; color: #1e40af; border: 1px solid #93c5fd; padding: 1px 4px; border-radius: 3px; font-weight: 800; font-size: 8px;">🌧️ TRABALHO SOB CHUVA</span> Sem Paralisação</span>
              <span style="display: inline-flex; align-items: center; gap: 3px;"><span style="background: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; padding: 1px 4px; border-radius: 3px; font-weight: 800; font-size: 8px;">☀️ PRODUTIVO / NORMAL</span> Tempo Limpo</span>
              <span style="display: inline-flex; align-items: center; gap: 3px;"><span style="color: #64748b; font-weight: 800; font-size: 8px;">⚪ FIM DE SEMANA</span> Sáb / Dom</span>
            </div>
          </div>

          <!-- Weekday Header -->
          <div style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; text-align: center; font-weight: 900; font-size: 9px; text-transform: uppercase; margin-bottom: 4px;">
            <div style="background: #f1f5f9; padding: 4px; border-radius: 4px; color: #64748b;">DOM</div>
            <div style="background: #e2e8f0; padding: 4px; border-radius: 4px; color: #334155;">SEG</div>
            <div style="background: #e2e8f0; padding: 4px; border-radius: 4px; color: #334155;">TER</div>
            <div style="background: #e2e8f0; padding: 4px; border-radius: 4px; color: #334155;">QUA</div>
            <div style="background: #e2e8f0; padding: 4px; border-radius: 4px; color: #334155;">QUI</div>
            <div style="background: #e2e8f0; padding: 4px; border-radius: 4px; color: #334155;">SEX</div>
            <div style="background: #f1f5f9; padding: 4px; border-radius: 4px; color: #64748b;">SÁB</div>
          </div>
  ...
  const handlePrintPluviometry = () => {
    const htmlContent = getPluviometryHTMLReport();
    const win = window.open('', '_blank', 'width=1100,height=900');
    if (!win) {
      alert('Por favor, permita popups no seu navegador para abrir a tela de impressão.');
      return;
    }
  ...
  const handleExportPluviometryPDF = async () => {
    const htmlContent = getPluviometryHTMLReport();
    ...
    try {
      ...
      pdf.save(`Relatorio_Pluviometrico_${contract.contractNumber || 'SYNERA'}_${currentYear}_${currentMonth + 1}.pdf`);
    } catch (err) {
      console.error('[PDF] Erro ao exportar PDF via html2canvas:', err);
      handlePrintPluviometry();
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={v => setActiveTab(v as any)} className="w-full">
        <TabsList className="mb-4 bg-gray-100 p-1 rounded-xl">
          <TabsTrigger value="calendar" className="flex items-center gap-2 rounded-lg font-black uppercase text-xs tracking-widest px-4 py-2">
            <Calendar className="w-4 h-4 text-blue-600" /> Calendário Mensal
          </TabsTrigger>
        ...
        <TabsContent value="calendar" className="space-y-4">
          {/* Header Legend and Tip */}
          <Card className="p-4 border border-gray-100 shadow-sm rounded-2xl bg-gradient-to-r from-slate-50 to-white">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 text-slate-600 font-bold">
                <span className="p-1.5 bg-blue-100 text-blue-700 rounded-lg">💡</span>
                <span><strong>Dica:</strong> Clique duas vezes (2x) sobre qualquer dia para abrir o modal e registrar o status pluviométrico.</span>
              </div>
              <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase">
                <div className="flex items-center gap-1.5 bg-red-50 text-red-700 px-2 py-1 rounded-md border border-red-200">
                  <span className="w-2.5 h-2.5 rounded-full bg-red-600"></span>
                  <span>Impraticável</span>
                </div>
                <div className="flex items-center gap-1.5 bg-blue-50 text-blue-700 px-2 py-1 rounded-md border border-blue-200">
                  <span className="w-2.5 h-2.5 rounded-full bg-blue-500"></span>
                  <span>Trabalho sob Chuva</span>
                </div>
          ...
          <Card className="p-4 border border-gray-100 shadow-sm rounded-2xl bg-white overflow-hidden">
            {/* Weekday Header */}
            <div className="grid grid-cols-7 gap-2 mb-2 text-center">
              {['DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB'].map((dayName, idx) => (
                <div 
                  key={dayName} 
                  className={cn(
                    "py-2 font-black text-xs uppercase tracking-wider rounded-xl border border-slate-100",
                    idx === 0 || idx === 6 ? "bg-slate-100/70 text-slate-500" : "bg-slate-50 text-slate-700"
                  )}
                >
                  {dayName}
                </div>
            ...
            <div className="grid grid-cols-7 gap-2 sm:gap-2.5">
              ...
              {Array.from({ length: new Date(currentYear, currentMonth, 1).getDay() }).map((_, idx) => (
                <div key={`empty-${idx}`} className="bg-slate-50/40 border border-dashed border-slate-200/50 rounded-2xl min-h-[128px] opacity-30 pointer-events-none" />
              ))}

              {/* Month Days */}
              {monthDays.map(day => {
                const record = getRecordForDay(day);
                const night = record?.nightStatus || 'Bom';
                const morning = record?.morningStatus || 'Bom';
                const afternoon = record?.afternoonStatus || 'Bom';
                const rainfall = record?.rainfallMm || 0;
                ...
                let cellBg = "bg-white border-slate-200 text-slate-800 hover:border-blue-400 hover:shadow-md";
                let impactBadge = <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 font-bold text-[9px] uppercase px-1.5 py-0.5">☀️ Produtivo</Badge>;

                if (isImpraticable) {
                  cellBg = "bg-red-500/10 border-2 border-red-400 text-red-950 hover:bg-red-500/20 shadow-sm shadow-red-100/50 hover:border-red-500";
                  impactBadge = <Badge className="bg-red-600 text-white font-extrabold text-[9px] uppercase px-1.5 py-0.5 tracking-wider shadow-xs">🛑 Impraticável</Badge>;
                } else if (isRainy) {
                  cellBg = "bg-blue-500/10 border-2 border-blue-300 text-blue-950 hover:bg-blue-500/20 shadow-sm shadow-blue-100/50 hover:border-blue-400";
                  impactBadge = <Badge variant="outline" className="bg-blue-100 text-blue-800 border-blue-300 font-extrabold text-[9px] uppercase px-1.5 py-0.5 tracking-wider shadow-xs">🌧️ Sob Chuva</Badge>;
                } else if (isWeekend) {
                  cellBg = "bg-slate-100/80 border border-slate-200 text-slate-600 hover:bg-slate-200/80";
                  impactBadge = <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">Fim de Semana</span>;
                }

                if (isToday) {
                  cellBg += " ring-2 ring-blue-600 ring-offset-2 border-blue-600 shadow-md shadow-blue-500/20";
                }
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "relative min-h-[132px] p-2.5 rounded-2xl border transition-all duration-300 flex flex-col justify-between cursor-pointer group select-none hover:-translate-y-1.5 hover:shadow-xl hover:z-20 hover:scale-[1.03]",
                      cellBg
                    )}
                    title="Clique duas vezes para editar os dados do dia"
                  >
                    {/* Top row */}
                    <div className="flex items-start justify-between gap-1">
                      <div className="flex items-center gap-1.5">
                        <span className={cn(
                          "text-base font-black font-mono tracking-tight transition-all",
                          isToday ? "bg-blue-600 text-white px-2 py-0.5 rounded-lg shadow-xs" : "group-hover:text-blue-600 group-hover:scale-110"
                        )}>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-5 w-5 opacity-0 group-hover:opacity-100 transition-all duration-200 text-slate-400 hover:text-blue-600 p-0 hover:scale-125"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleOpenModal(day);
                          }}
                    ...
                    {/* Middle row: Night, Morning, Afternoon status */}
                    <div className="my-1.5 grid grid-cols-3 gap-1 text-center bg-white/80 backdrop-blur-xs p-1.5 rounded-xl border border-black/5 group-hover:border-blue-300 group-hover:bg-white group-hover:shadow-sm transition-all duration-300">
                      <div className="flex flex-col items-center">
                        <span className="text-[8px] font-black uppercase text-slate-400 group-hover:text-slate-600 transition-colors">Noite</span>
                        <span className="text-sm transition-transform duration-300 inline-block group-hover:scale-125 group-hover:rotate-12">
                          {night === 'Bom' ? '☀️' : night === 'Chuvoso' ? '🌧️' : '🛑'}
                        </span>
                      </div>
                      <div className="flex flex-col items-center border-x border-slate-200/60">
                        <span className="text-[8px] font-black uppercase text-slate-400 group-hover:text-slate-600 transition-colors">Manhã</span>
                        <span className="text-sm transition-transform duration-300 inline-block group-hover:scale-125 group-hover:-translate-y-0.5">
                          {morning === 'Bom' ? '☀️' : morning === 'Chuvoso' ? '🌧️' : '🛑'}
                        </span>
                      </div>
                      <div className="flex flex-col items-center">
                        <span className="text-[8px] font-black uppercase text-slate-400 group-hover:text-slate-600 transition-colors">Tarde</span>
                        <span className="text-sm transition-transform duration-300 inline-block group-hover:scale-125 group-hover:-rotate-12">
                          {afternoon === 'Bom' ? '☀️' : afternoon === 'Chuvoso' ? '🌧️' : '🛑'}
                        </span>
                      </div>
                    </div>

                    {/* Bottom row: Precipitation mm */}
                    <div className="flex items-center justify-between text-[10px] font-bold">
                      {rainfall > 0 ? (
                        <span className="inline-flex items-center gap-1 text-blue-700 font-mono font-black bg-blue-100/90 px-1.5 py-0.5 rounded-md group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shadow-xs">
                          <CloudRain className="w-3 h-3 text-blue-600 group-hover:text-white group-hover:animate-bounce" />
                          {rainfall} mm
                        </span>
                      ) : (
                        <span className="text-[9px] text-slate-400 font-medium group-hover:text-slate-600 transition-colors">0.0 mm</span>
                      )}
                    </div>
                  </div>
                );
      ...
      {/* Modal Dialog for Double-Click editing */}
      <Dialog open={selectedDayForModal !== null} onOpenChange={(open) => { if (!open) setSelectedDayForModal(null); }}>
        <DialogContent className="max-w-md rounded-2xl p-6 bg-white border border-slate-200 shadow-2xl z-[100]">
          <DialogHeader className="pb-3 border-b border-slate-100">
            <DialogTitle className="text-base font-black uppercase text-slate-900 flex items-center gap-2">
              <Calendar className="w-5 h-5 text-blue-600" />
              Lançamento Pluviométrico
            </DialogTitle>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mt-1">
              {selectedDayForModal && `${String(selectedDayForModal).padStart(2, '0')} de ${['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'][currentMonth]} de ${currentYear}`}
            </p>
          </DialogHeader>
          ...
          <div className="space-y-4 py-3">
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Preenchimento Rápido</Label>
              <div className="grid grid-cols-3 gap-2">
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  className="text-[10px] font-bold uppercase tracking-wider border-emerald-200 hover:bg-emerald-50 text-emerald-700 h-8"
                  onClick={() => {
                    setModalNightStatus('Bom');
                    setModalMorningStatus('Bom');
                    setModalAfternoonStatus('Bom');
                    setModalRainfallMm(0);
                  }}
                >
                  ☀️ Bom / Sol
                </Button>
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="text-[10px] font-bold uppercase tracking-wider border-blue-200 hover:bg-blue-50 text-blue-700 h-8"
                  onClick={() => {
                    setModalNightStatus('Bom');
                    setModalMorningStatus('Chuvoso');
                    setModalAfternoonStatus('Chuvoso');
                    if (modalRainfallMm === 0) setModalRainfallMm(10);
                  }}
                >
                  🌧️ Sob Chuva
                </Button>
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="text-[10px] font-bold uppercase tracking-wider border-red-200 hover:bg-red-50 text-red-700 h-8"
                  onClick={() => {
                    setModalNightStatus('Chuvoso');
                    setModalMorningStatus('Impraticável');
                    setModalAfternoonStatus('Impraticável');
                    if (modalRainfallMm === 0) setModalRainfallMm(25);
                  }}
                >
                  🛑 Impraticável
                </Button>
            ...
            {/* 1. Noite Anterior */}
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-700">1. Noite Anterior</Label>
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200/80">
                {(['Bom', 'Chuvoso', 'Impraticável'] as const).map((st) => (
                  <button
                    key={`night-${st}`}
                    type="button"
                    disabled={readonly}
                    onClick={() => setModalNightStatus(st)}
                    className={cn(
                      "py-2 px-2 text-[11px] font-extrabold rounded-lg transition-all flex items-center justify-center gap-1 uppercase tracking-wider cursor-pointer select-none",
                      modalNightStatus === st
                        ? st === 'Bom'
                  ...
                  >
                    <span>{st === 'Bom' ? '☀️ Bom' : st === 'Chuvoso' ? '🌧️ Chuvoso' : '🛑 Impraticável'}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* 2. Manhã */}
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-700">2. Manhã</Label>
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200/80">
                {(['Bom', 'Chuvoso', 'Impraticável'] as const).map((st) => (
                  <button
                    key={`morning-${st}`}
                    type="button"
                    disabled={readonly}
                    onClick={() => setModalMorningStatus(st)}
                    className={cn(
                      "py-2 px-2 text-[11px] font-extrabold rounded-lg transition-all flex items-center justify-center gap-1 uppercase tracking-wider cursor-pointer select-none",
                      modalMorningStatus === st
                        ? st === 'Bom'
                          ? "bg-emerald-600 text-white shadow-sm"
                          : st === 'Chuvoso'
                          ? "bg-blue-600 text-white shadow-sm"
            ...
            {/* 3. Tarde */}
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-700">3. Tarde</Label>
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200/80">
                {(['Bom', 'Chuvoso', 'Impraticável'] as const).map((st) => (
                  <button
                    key={`afternoon-${st}`}
                    type="button"
                    disabled={readonly}
                    onClick={() => setModalAfternoonStatus(st)}
                    className={cn(
                      "py-2 px-2 text-[11px] font-extrabold rounded-lg transition-all flex items-center justify-center gap-1 uppercase tracking-wider cursor-pointer select-none",
                      modalAfternoonStatus === st
                        ? st === 'Bom'
                          ? "bg-emerald-600 text-white shadow-sm"
                          : st === 'Chuvoso'
                          ? "bg-blue-600 text-white shadow-sm"
            ...
            {/* Volume Chuva mm */}
            <div className="space-y-1.5">
              <Label className="text-xs font-bold uppercase text-slate-700">4. Precipitação Pluviométrica</Label>
              <div className="relative">
                <Input
                  ...
                  min="0"
                  disabled={readonly}
                  className="h-10 text-sm font-mono font-black rounded-xl pr-12 focus:ring-2 focus:ring-blue-500"
                  value={modalRainfallMm}
                  onChange={(e) => setModalRainfallMm(parseFloat(e.target.value) || 0)}
                />
            ...
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
              <span className="text-[10px] font-black uppercase text-slate-500">Impacto Estimado:</span>
              {modalMorningStatus === 'Impraticável' || modalAfternoonStatus === 'Impraticável' ? (
                <Badge className="bg-red-600 text-white font-extrabold text-[10px] uppercase shadow-xs">🛑 Impraticável</Badge>
              ) : modalMorningStatus === 'Chuvoso' || modalAfternoonStatus === 'Chuvoso' || modalNightStatus === 'Chuvoso' || modalRainfallMm > 0 ? (
                <Badge variant="outline" className="text-blue-700 border-blue-300 bg-blue-50 font-extrabold text-[10px] uppercase shadow-xs">🌧️ Trabalho sob chuva</Badge>
              ) : (
                <Badge variant="outline" className="text-emerald-700 border-emerald-300 bg-emerald-50 font-extrabold text-[10px] uppercase shadow-xs">☀️ Produtivo / Normal</Badge>
              )}
            </div>
          </div>

          <DialogFooter className="gap-2 sm:gap-0 pt-2 border-t border-slate-100">
            <Button variant="ghost" size="sm" onClick={() => setSelectedDayForModal(null)} className="font-bold uppercase text-xs">
              Cancelar
            </Button>
            {!readonly && (
              <Button size="sm" onClick={handleSaveModal} className="bg-blue-600 hover:bg-blue-700 text-white font-black uppercase text-xs tracking-wider gap-1.5 rounded-xl px-5">
