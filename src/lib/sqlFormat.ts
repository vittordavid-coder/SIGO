...
export function generateFullSQLScript(data: {
  ...
  const tableDefs: Record<string, string> = {
    ...
    suppliers: `id TEXT PRIMARY KEY, company_id TEXT, name TEXT NOT NULL, category TEXT, activity TEXT, cnpj TEXT, contact_person TEXT, email_website TEXT, phone TEXT, mobile TEXT, address TEXT, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    purchase_requests: `id TEXT PRIMARY KEY, company_id TEXT, date DATE, description TEXT, sector TEXT, category TEXT, status TEXT DEFAULT 'Aguardando', items JSONB DEFAULT '[]', created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    ...
    purchase_orders: `id TEXT PRIMARY KEY, company_id TEXT, order_number TEXT, supplier_id TEXT, supplier_name TEXT, order_date DATE, total NUMERIC DEFAULT 0, status TEXT DEFAULT 'draft', items JSONB DEFAULT '[]', payment_condition JSONB DEFAULT '{}', delivery_date DATE, tracking_number TEXT, notes TEXT, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`
  };
