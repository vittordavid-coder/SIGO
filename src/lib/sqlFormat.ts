...
export const DB_TABLES = [
  ...
  'purchase_order_items', 'purchase_order_payments', 'fuel_reservoirs', 'fuel_logs', 'rh_templates', 'aportes', 'aporte_items', 'field_reports'
];




  export const SUPABASE_TABLE_DEFS: Record<string, string> = {
    ...
    field_reports: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT, contract_name TEXT, service_id TEXT, service_name TEXT, unit TEXT, qty NUMERIC DEFAULT 0, production_date DATE, synced_at TIMESTAMPTZ, start_station TEXT, end_station TEXT, trecho TEXT, notes TEXT, photo TEXT, photo_url TEXT, reported_by TEXT, reported_by_email TEXT, status TEXT DEFAULT 'pending', rejected_by TEXT, rejection_reason TEXT, rejected_at TIMESTAMPTZ, approved_by TEXT, approved_at TIMESTAMPTZ, info_type TEXT, trips_qty NUMERIC DEFAULT 0, length_m NUMERIC DEFAULT 0, width_m NUMERIC DEFAULT 0, height_m NUMERIC DEFAULT 0, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    ...
    resources: `id TEXT PRIMARY KEY, company_id TEXT, code TEXT NOT NULL, name TEXT NOT NULL, unit TEXT, type TEXT, base_price NUMERIC DEFAULT 0, encargos NUMERIC DEFAULT 0, productive_price NUMERIC DEFAULT 0, unproductive_price NUMERIC DEFAULT 0, operator_id TEXT, equipment_base_cost NUMERIC DEFAULT 0, hours_per_month NUMERIC DEFAULT 220, monthly_salary NUMERIC DEFAULT 0, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    service_compositions: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT, code TEXT NOT NULL, name TEXT NOT NULL, unit TEXT, production NUMERIC DEFAULT 1, fit NUMERIC DEFAULT 1, items JSONB DEFAULT '[]', created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    ...
    station_groups: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT REFERENCES contracts(id) ON DELETE CASCADE, name TEXT, initial_station TEXT, final_station TEXT, material_ids JSONB DEFAULT '[]', volume NUMERIC, service_id TEXT, operation_type TEXT CHECK (operation_type IN ('corte', 'aterro', '')), created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    ...
    team_assignments: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT REFERENCES contracts(id) ON DELETE CASCADE, team_id TEXT REFERENCES controller_teams(id) ON DELETE CASCADE, member_id TEXT NOT NULL, type TEXT NOT NULL, month TEXT, start_date DATE, end_date DATE, created_at TIMESTAMPTZ DEFAULT now()`,
    ...
    rh_templates: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT, name TEXT NOT NULL, type TEXT, file_data TEXT, created_at TIMESTAMPTZ DEFAULT now()`,
    ...
    fuel_logs: `id TEXT PRIMARY KEY, company_id TEXT, tank_id TEXT, type TEXT NOT NULL, date DATE NOT NULL, quantity NUMERIC NOT NULL, equipment_id TEXT, notes TEXT, unit_price NUMERIC, cost NUMERIC, supplier TEXT, invoice_number TEXT, hour_meter NUMERIC, created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
    aportes: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT, numero VARCHAR(255) NOT NULL, data DATE DEFAULT CURRENT_DATE, created_at TIMESTAMPTZ DEFAULT now()`,
    aporte_items: `id TEXT PRIMARY KEY, aporte_id TEXT REFERENCES public.aportes(id) ON DELETE CASCADE, categoria VARCHAR(255), subcategoria VARCHAR(255), fornecedor VARCHAR(255), descricao TEXT, mes_competencia VARCHAR(50), data_vencimento DATE, valor NUMERIC DEFAULT 0, purchase_order_id TEXT, created_at TIMESTAMPTZ DEFAULT now()`
  };
...
export function generateFullSQLScript(data: {
  ...
  DB_TABLES.forEach(table => {
    if (SUPABASE_TABLE_DEFS[table]) {
      sql += `CREATE TABLE IF NOT EXISTS ${table} (\n  ${SUPABASE_TABLE_DEFS[table]}\n);\n`;
    }
  });
...
CREATE TABLE IF NOT EXISTS resources (
  ...
  operator_id TEXT,
  equipment_base_cost NUMERIC DEFAULT 0,
  hours_per_month NUMERIC DEFAULT 220,
...
ALTER TABLE resources ADD COLUMN IF NOT EXISTS operator_id TEXT;
ALTER TABLE resources ADD COLUMN IF NOT EXISTS equipment_base_cost NUMERIC DEFAULT 0;
ALTER TABLE resources ADD COLUMN IF NOT EXISTS hours_per_month NUMERIC DEFAULT 220;
ALTER TABLE resources ADD COLUMN IF NOT EXISTS monthly_salary NUMERIC DEFAULT 0;
