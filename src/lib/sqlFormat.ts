...
export function generateFullSQLScript(data: {
  ...
  const tableDefs: Record<string, string> = {
    ...
    purchase_requests: `id TEXT PRIMARY KEY, company_id TEXT, contract_id TEXT, date DATE, description TEXT, sector TEXT, category TEXT, priority TEXT DEFAULT 'Normal', status TEXT DEFAULT 'Aguardando', items JSONB DEFAULT '[]', created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()`,
